<template>
    <div @click="checkClick($event)">
        ...
        <b-container class="stats-page">
            ...
            <div
                class="posts d-flex"
                v-if="lastestTwoPosts && lastestTwoPosts.length"
            >
                <PostTeaser
                    class="col-md-6"
                    v-for="post in lastestTwoPosts"
                    :post="post"
                    :key="post.id"
                />
            </div>
...
export default {
    ...
    computed: {
        ...
        ...mapGetters('posts', ['lastestTwoPosts']),
    },
    methods: {
        checkClick(e) {
            const dropdownAll = this.$el.querySelectorAll(
        ...
        getCharts() {
            this.$el
                .querySelectorAll('.visualization-chart h2 a')
                .forEach((el) => {
