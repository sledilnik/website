...
type DisplayType =
    ...
    | ByWeekUsage
    | ByWeekSupply
    static member All = [ Used ; Delivered; Unused; ByWeekUsage; ByWeekSupply; ]
...
let renderStackedChart state dispatch =
    ...
    let baseOptions =
        basicChartOptions Linear "covid19-vaccines-stacked"
...
let renderWeeklyChart state dispatch =
    ...
    let baseOptions =
        basicChartOptions Linear "covid19-vaccines-weekly"
            state.RangeSelectionButtonIndex
            onRangeSelectorButtonClick
    {| baseOptions with
        series = Seq.toArray allSeries
