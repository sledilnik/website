...
type GeneMeasurement = { raw: float; norm: float }

type CasesEstimate = { estimated: float; active100k: int }
    ...
    let fetch () =
        async {
            ...
            return
                ...
                | 200 ->
                    let data =
                        json |> SimpleJson.parseNative |> Json.tryConvertFromJsonAs<SewageCases array>
