...
function mapToStateItem(obj) {
    obj['valid_since'] = obj['valid_since']
        ? new Date(obj['valid_since'])
        : null
    obj['valid_until'] = obj['valid_until']
        ? new Date(obj['valid_until'])
        : null
    return obj
}
...
const actions = {
    ...
    async fetch({ commit }, { limit }) {
        const { meta, objects } = await contentApi.get(
...
const mutations = {
    [FRESH_RESTRICTIONS](state, restrictions) {
        state.restrictions = unionBy(
            map(restrictions, mapToStateItem),
            state.restrictions,
            'id'
        )
    },
